...
>
  <div class="mx-auto mt-24 md:w-150 bg-bgColor dark:bg-bgColordark rounded-lg" id="searchbar">
    <header class="px-4 pb-2 pt-4">
      ...
      text-textColor2 dark:text-textColor2dark border-none outline-none pa-3"
        ...
        {:else}
          <div
            aria-label="Clear Input"
            class="text-3xl ml-3 i-mdi-close text-textColor2 dark:text-textColor2dark mr-3 opacity-0"
          ></div>
        {/if}
      </form>
    </header>
          ...
          {#each result as item (item.meta.title)}
            <li class="rounded-lg bg-bgColor2 dark:bg-bgColor2dark my-2 h-30 group">
              <a href={item.url} class="w-full h-full px-4 py-2 flex flex-col no-underline" onclick={hiddenSearchBar}>
                <h3
                  class="my-1 text-lg font-bold text-textColor2 dark:text-textColor2dark a-300 group-hover:text-themeColor"
                >
                  {item.meta.title}
                </h3>
    ...
    <footer class="flex justify-end items-center bg-bgColor2 dark:bg-bgColor2dark px-3 rounded-lb-lg rounded-rb-lg">
      <p class="i-mdi-keyboard-esc text-textColor2 dark:text-textColor2dark"></p>
      <p class="text-sm ml-3 text-textColor2 dark:text-textColor2dark">to close</p>
    </footer>
  </div>
</div>
